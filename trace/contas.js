...
/* Entrada por provedor (SSO): cria a conta na primeira vez, garante o vinculo,
 * e NUNCA rebaixa quem ja foi promovido a mao. Senha local nasce impossivel de
 * adivinhar, entao a conta so entra pelo provedor. */
function entrarPorProvedor(req, { email, tenantId, papel }) {
  const e = String(email || '').trim().toLowerCase();
  let u = banco.usuarioPorEmail(e);
  if (!u) u = banco.criarUsuario(e, hashSenha(crypto.randomBytes(32).toString('base64url')));
  const atual = banco.vinculo(tenantId, u.id);
  if (!atual) banco.vincular(tenantId, u.id, papel || 'consultor');
  banco.registrar(tenantId, u.id, 'entrar-sso', null, ipDe(req));
  return abrirSessao(req, u, tenantId);
}
...
module.exports = { hashSenha, conferirSenha, cadastrar, entrar, entrarPorProvedor, sessaoDe, sair, ipDe, COOKIE, DURACAO_MS, MAX_TENTATIVAS };
